...
class CheckoutService {
  ...
  Future<List> listPlacedOrder() async {
    List orderList = [];
    String uid = await _userService.getUserId();
    // QuerySnapshot orderstimes = await _orderReference.orderBy('placedDate',descending: true).where('userId', isEqualTo: uid).get();
    // print("test ${orderstimes}");
    // for(DocumentSnapshot ordertime in orderstimes.docs){
    //   print("time ${ordertime}");
    // }
    QuerySnapshot orders = await _orderReference.where('userId', isEqualTo: uid).orderBy('placedDate',descending: true).get();
    print("order ne $orders");
